...
import pretrain.scripts.butil
from pretrain.scripts.model import Sequence
def get_sub_seqs(s, numSeg, evalType):
    ...
    rect_anno = s.gtRect
    subSeqs, subAnno = pretrain.scripts.butil.split_seq_TRE(s, numSeg, rect_anno)
    s.subAnno = subAnno
    ...
    if evalType == 'OPE':
        ...
        for i in range(len(shiftTypeSet)):
            ...
            s.init_rect = pretrain.scripts.butil.shift_init_BB(s.init_rect, shiftType,
                                                      imgHeight, imgWidth)
...
def save_seq_config(seq):
    # print(dict(seq.__dict__))
    temp = dict(seq.__dict__)
    for i,j in enumerate(temp['gtRect']):
        temp['gtRect'][i] = list(j)
    # print(temp)
    # print(list(temp))
    string = json.dumps(temp, indent=2)
    src = os.path.join(SEQ_SRC_TEMP, seq.name)
    configFile = open(src+'/cfg.json', 'wb')
    configFile.write(string.encode())
    configFile.close()

def load_seq_config(seqName):
    src = os.path.join(SEQ_SRC_TEMP, seqName)
...
def download_sequence(seqName):
    file_name = SEQ_SRC_TEMP + seqName + '.zip'

    if seqName == 'Jogging-1' or seqName == 'Jogging-2':
        url = DOWNLOAD_URL.format('Jogging')
        download_and_extract_file(url, file_name, SEQ_SRC_TEMP)
        src = SEQ_SRC_TEMP + 'Jogging/'
        dst1 = SEQ_SRC_TEMP + 'Jogging-1/'
        dst2 = SEQ_SRC_TEMP + 'Jogging-2/'
        ...
        download_and_extract_file(url, file_name, SEQ_SRC_TEMP)
        src = SEQ_SRC_TEMP + 'Skating2/'
        dst1 = SEQ_SRC_TEMP + 'Skating2-1/'
        dst2 = SEQ_SRC_TEMP + 'Skating2-2/'
        if not os.path.exists(dst1 + 'img'):
            shutil.copytree(src + 'img', dst1 + 'img')
        if not os.path.exists(dst2 + 'img'):
            shutil.copytree(src + 'img', dst2 + 'img')
        ...
        download_and_extract_file(url, file_name, SEQ_SRC_TEMP)
        src = SEQ_SRC_TEMP + 'Human4/'
        # dst1 = SEQ_SRC + 'Human4-1/'
        dst2 = SEQ_SRC_TEMP + 'Human4-2/'
        # if not os.path.exists(dst1 + 'img'):
        #     shutil.copytree(src + 'img', dst1 + 'img')
        if not os.path.exists(dst2 + 'img'):
            shutil.copytree(src + 'img', dst2 + 'img')
    ...
    if os.path.exists(SEQ_SRC_TEMP + '__MACOSX'):
        shutil.rmtree(SEQ_SRC_TEMP + '__MACOSX')
