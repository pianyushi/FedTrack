...
def crop_image2(img, bbox, img_size=107, padding=16, flip=False, rotate_limit=0, blur_limit=0):  # 从img中选出bbox区域的图片,resize为107*107的
...
def crop_image(img, bbox, img_size=107, padding=16, valid=False):
    ...
    scaled = np.array(Image.fromarray(cropped), np.resize(img_size, img_size))
    return scaled
